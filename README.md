...
The active-round cache and pending mutations are saved atomically in `golf-state.json` under an account-specific Application Support directory. Only the merge bases needed for those pending changes are retained; round history and server cursors stay in memory. Existing JSON files and the practice UserDefaults journal are migrated when the first account signs in; an ownership marker prevents importing those records into another account. Sign-out switches to a separate guest cache. Saved local changes are the durable offline queue.

Cloud data lives in typed `rounds`, `hole_scores`, `shots`, `clubs`, and `practice_sessions` tables. Composite ownership/parent foreign keys and owner-only RLS isolate accounts. `commit_golf_records` atomically compares each changed record’s revision; unrelated edits can both succeed. `pull_golf_records` serves the current cloud data in pages; history requests start from zero and keep the response only in memory. That account row contains only sync metadata, not golf data. Deletes cascade tombstones to child records, preventing stale devices from resurrecting removed rounds. Known score/shot fields are ordinary columns; evolving observations and course geometry remain JSONB. Old `golf_sync_state` and `rounds_legacy_snapshot` payloads are retained as read-only migration sources; migration verifies entity counts and score totals before cutover. Old snapshot writes require an app upgrade. Cloud requests run when Me opens or changes section, on pull-to-refresh, after edits (debounced), on sign-in/foregrounding, or Account → Sync golf data now. Failed requests retry with exponential backoff from 2 to 60 seconds while the app is running; a foreground check also resumes pending work every 30 seconds; acknowledged history is not polled. A failed history request shows a retry state rather than saved history. Disk holds only the active round (including its bag) and unacknowledged changes with their merge bases; acknowledged history and cursor checkpoints are never persisted. Existing full caches are compacted on load without losing pending edits. Offline in-round changes remain durable and retry on those triggers. Devices must use the same account. The auth client emits its cached session before attempting refresh so offline restarts can open the correct cache.

Live edits coalesce behind a single in-flight upload. The exact batch, UUID, and submitted baseline are persisted before sending. `commit_golf_batch` stores an account-scoped receipt atomically with the revision-checked writes; lost responses and app restarts replay the same batch ID. Newer local edits survive acknowledgements and become the next batch. Unconfirmed finished rounds stay on disk. Deploy `20260917090000_idempotent_golf_uploads.sql` before distributing this client; without the RPC, uploads stay queued locally and report the backend error. Receipts must be retained while an offline client could still replay its batch.
...
Validation: `PINPOINT_SMOKE_MAIN=tools/UploadQueueSmokeMain.swift bash tools/run-smoke.sh` covers lost responses, restart recovery, racing edits, rejected uploads, and acknowledgement-based cleanup. `bash tools/run-smoke.sh` includes offline persistence, account isolation, cloud response assembly, and merge checks. `PINPOINT_SMOKE_MAIN=tools/SyncSchedulingMain.swift bash tools/run-smoke.sh` verifies cancellation isolation and coalesced requests. `PINPOINT_SYNC_FIXTURE=/tmp/pinpoint-sync-fixture.json bash tools/run-smoke.sh` generates real model fixtures. `python3 tools/test-golf-sync-local.py` checks actual local Auth/RLS, migration, independent/concurrent record revisions, delta reads, and cascading deletions; `--deployed` runs against the explicitly linked Pinpoint project using temporary QA accounts, which are deleted afterwards. Administrative credentials stay only in process memory and never enter the app bundle.
